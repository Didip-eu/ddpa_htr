...
class TorchVGSLModel(object):
    ...
    def load_model(cls, path: Union[str, PathLike]):
        ...
        try:
            ...
            raise InvalidModelException('Failure parsing model protobuf: {}'.format(str(e))) from e
        if 'vgsl' not in mlmodel.user_defined_metadata:
            raise InvalidModelException('No VGSL spec in model metadata')
        ...
        try:
            ...
            raise InvalidModelException('Failed parsing out layers from model weights') from exc
