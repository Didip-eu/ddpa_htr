...
def line_polygons_from_segmentation_dict( segmentation_dict: dict, factor=1.0 ) -> list[list[int]]:
    ...
    for line in flat_dict['lines']:
        ...
        polygon_pts = polygon_utils.strip_from_baseline( line['baseline'], line['x-height'], factor, ltrb=ltrb ) if 'x-height' in line else line['coords'] 
        line_polygons.append( polygon_pts )
